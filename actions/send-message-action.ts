...
import { transporter } from "@/app/api/nodemailerConfig";
...
export default async function sendMessageAcion(formData: FormData) {
  ...
  try {
    ...
    if (mailMessage.rejected.length > 0) {
      return { success: false, message: "Erro ao enviar o email", status: 500 };
    }

    const confirmationMessage = await transporter.sendMail({
...
`,
    ...
    if (confirmationMessage.rejected.length > 0)
      return {
        success: false,
